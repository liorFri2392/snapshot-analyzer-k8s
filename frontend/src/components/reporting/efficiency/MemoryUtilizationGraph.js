...
const MemoryUtilizationGraph = ({ data, phase2Date, baseline }) => {
    ...
    useEffect(() => {
        ...
        if (phase2Date) {
            ...
            const preBaseline = {
                ...
                average: preAutomationData.map(item => {
                    const used = parseFloat(item.ramUsedGibOnDemand) + parseFloat(item.ramUsedGibSpot) + parseFloat(item.ramUsedGibSpotFallback);
                    const total = parseFloat(item.ramGibOnDemand) + parseFloat(item.ramGibSpot) + parseFloat(item.ramGibSpotFallback);
                    return total > 0 ? (used / total) * 100 : 0;
        ...
        setLayout({
            title: {
                text: 'Memory Utilization by Instance Type',
                font: { size: 16 }
            },
            xaxis: {
                title: 'Date',
