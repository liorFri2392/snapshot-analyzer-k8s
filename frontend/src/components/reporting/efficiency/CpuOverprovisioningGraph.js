...
const CpuOverprovisioningGraph = ({ data, phase2Date, baseline }) => {
    const [plotData, setPlotData] = useState([]);
    const [layout, setLayout] = useState({});

    useEffect(() => {
        console.log('CpuOverprovisioningGraph received data:', data);
        console.log('CpuOverprovisioningGraph received phase2Date:', phase2Date);
        if (!data || !data.items) {
        ...
        setLayout({
            title: {
                text: 'CPU Overprovisioning by Instance Type',
                font: { size: 16 }
            },
            xaxis: {
                title: 'Date',
    ...
    if (!data || !data.items) {
        console.log('CpuOverprovisioningGraph: No data available');
...
export default CpuOverprovisioningGraph; 