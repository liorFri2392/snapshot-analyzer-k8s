...
const RoleBindingDetails = ({ items = [] }) => {
  ...
  const namespaces = [...new Set(items.map(binding => binding.metadata.namespace))].sort();

  // Filter and paginate rolebindings
  const filteredBindings = useMemo(() => {
    return items.filter(binding => {
      const matchNamespace = !filters.namespace || binding.metadata.namespace === filters.namespace;
  ...
  if (!items?.length) {
    return (
      <div className="bg-white p-4 rounded-lg text-center text-gray-500">
        No rolebinding details available
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="text-sm text-gray-500 mb-2">
        Showing {paginatedBindings.length} of {filteredBindings.length} rolebindings
      </div>
        ...
        <tbody>
          {subjects.map((subject, index) => (
            <tr key={index} className="border-t">
              ...
              <td className="py-2 px-3">{subject.namespace || 'N/A'}</td>
            </tr>
          ))}
        </tbody>
      </table>
