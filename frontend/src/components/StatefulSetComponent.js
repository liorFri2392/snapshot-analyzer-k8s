...
const StatefulSetStatus = ({ status }) => {
  const getStatusColor = () => {
    if (status.readyReplicas === status.replicas && status.currentReplicas === status.replicas) {
...
const StatefulSetDetails = ({ items = [] }) => {
  ...
  // Filter and paginate statefulsets
  ...
  if (!items?.length) {
    return (
      <div className="bg-white p-4 rounded-lg text-center text-gray-500">
        No statefulset details available
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="text-sm text-gray-500 mb-2">
        Showing {paginatedSets.length} of {filteredSets.length} statefulsets
      </div>
          ...
          >
            <div className="flex-grow">
              <div className="flex items-center justify-between">
                ...
                <StatefulSetStatus status={set.status} />
              </div>
            </div>
            ...
            <div>
              ...
              <div className="p-6">
                ...
                {getActiveTab(setIndex) === 'volumes' && (
                  <VolumesList
                    volumes={set.spec.template.spec.volumes}
        ...
        <table className="w-full text-left">
          <tbody>
            ...
            <tr className="border-t">
              ...
              <td className="py-2 px-3">{spec.serviceName}</td>
            </tr>
            <tr className="border-t">
              <td className="py-2 px-3 font-medium">Pod Management Policy</td>
              <td className="py-2 px-3">{spec.podManagementPolicy || 'OrderedReady'}</td>
            </tr>
            <tr className="border-t">
              <td className="py-2 px-3 font-medium">Update Strategy</td>
              <td className="py-2 px-3">{updateStrategy.type || 'RollingUpdate'}</td>
            </tr>
            {updateStrategy.rollingUpdate && (
              <tr className="border-t">
                <td className="py-2 px-3 font-medium">Partition</td>
                <td className="py-2 px-3">{updateStrategy.rollingUpdate.partition || 0}</td>
              </tr>
            )}
          </tbody>
        </table>
...
const StatusDetails = ({ status }) => {
  const details = [
    ...
    { label: 'Current Revision', value: status.currentRevision },
    { label: 'Update Revision', value: status.updateRevision },
...
const VolumesList = ({ volumes = [], volumeClaimTemplates = [] }) => {
  if (!volumes?.length && !volumeClaimTemplates?.length) {
    return (
      <div className="bg-white rounded-lg p-4 text-gray-500 text-center">
        No volumes found
      </div>
  ...
  return (
    <div className="space-y-6">
      ...
      {volumeClaimTemplates?.length > 0 && (
        <div>
          <h3 className="text-lg font-medium mb-4">Volume Claim Templates</h3>
          {volumeClaimTemplates.map((template, index) => (
            <div key={index} className="bg-white rounded-lg border mb-4">
              <div className="p-4 border-b bg-gray-50">
                <h4 className="font-medium">{template.metadata?.name || `Template ${index + 1}`}</h4>
              </div>
              <div className="p-4">
                <table className="w-full">
                  <tbody>
                    <tr>
                      <td className="py-2 font-medium">Storage Class</td>
                      <td className="py-2">{template.spec?.storageClassName || 'default'}</td>
                    </tr>
                    <tr>
                      <td className="py-2 font-medium">Access Modes</td>
                      <td className="py-2">{template.spec?.accessModes?.join(', ') || 'N/A'}</td>
                    </tr>
                    <tr>
                      <td className="py-2 font-medium">Resources</td>
                      <td className="py-2">
                        {template.spec?.resources?.requests?.storage || 'N/A'}
                      </td>
                    </tr>
                  </tbody>
                </table>
